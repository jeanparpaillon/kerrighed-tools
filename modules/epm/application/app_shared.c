...

{
	...
	while (s.index.type != NO_OBJ) {
		...
		DEBUG(DBG_APP_CKPT, 5, "Object %d:%lu (node %d) received\n",
		      s.index.type, s.index.key, s.node);
				    ...
				    enum shared_obj_type type)
{
	...
	DEBUG(DBG_APP_CKPT, 5, "%p: key: %lu, type: %d\n",
	      s, s->index.key, s->index.type);
			 ...
			 credentials_t *user_creds)
{
	...
	DEBUG(DBG_APP_CKPT, 3, "(%ld) Step 4 done\n",
	      app->app_id);
	...
	DEBUG(DBG_APP_CKPT, 3, "(%ld) Step 5 done\n", app->app_id);

	/* 6) receive updated information about shared objects */
	r = rcv_full_restored_objects(desc, app);

	DEBUG(DBG_APP_CKPT, 3, "(%ld) Step 6 done: %d\n", app->app_id, r);
