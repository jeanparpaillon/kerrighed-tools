/*
 *  Kerrighed/modules/epm/action.c
 *
...
static unsigned long action_to_flag_mapping[] = {
	[EPM_MIGRATE] = PF_MIGRATING,
	[EPM_REMOTE_CLONE] = PF_DISTANT_FORKING,
	[EPM_CHECKPOINT] = PF_CHECKPOINTING,
};
...
static inline unsigned long action_to_flag(krg_epm_action_t action)
{
	if (unlikely(action <= EPM_NO_ACTION || action >= EPM_ACTION_MAX))
		return 0;
	else
		return action_to_flag_mapping[action];
}
...
int krg_action_start(struct task_struct *task, krg_epm_action_t action)
{
	...
	if (!can_use_krg_cap(task, action_to_cap(action)))
		retval = -EPERM;
	else if (unlikely(task->flags & flag))
		retval = -EALREADY;
	else if (unlikely(task->flags & PF_KERRIGHED_ACTION))
		retval = -EAGAIN;
	else
		task->flags |= flag;
	action_lock_unlock();
