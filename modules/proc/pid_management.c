/*
 *  Kerrighed/modules/proc/pid_management.c
 *
			   ...
			   struct kddm_set *set, objid_t objid, int flags)
{
	...
	/* BUG_ON(ORIG_NODE(objid) != kerrighed_node_id); */ /* Can be false in case of restart */
...
static struct pid *no_pid(int nr)
{
	...
	spin_lock(&pid_kddm_lock);
	rcu_read_lock();
	pid = find_pid(nr); /* Double check once locked */
	rcu_read_unlock();
...
static struct pid *kcb_get_pid(int nr)
{
	struct pid_kddm_object *obj;
	struct pid *pid;
...
static void kcb_end_get_pid(struct pid *pid)
{
...
static int may_put_pid(struct pid_kddm_object *obj)
{
	...
	DEBUG(DBG_PID, 4, "%d attach_pending=%d active=%d\n",
	      pid->nr, obj->attach_pending, obj->active);
	...
	for (tmp = PIDTYPE_MAX; --tmp >= 0; )
		if (!hlist_empty(&pid->tasks[tmp])) {
			DEBUG(DBG_PID, 4, "type %d not empty (%d 0x%p)\n",
			      tmp, pid_task(pid, tmp)->pid, pid_task(pid, tmp));
			return 0;
		}
...
static void kcb_put_pid(struct pid *pid)
{
	struct pid_kddm_object *obj;
...
void unimport_pid_namespace(struct task_struct *task)
{
	put_pid_ns(task->nsproxy->pid_ns);
}
...
static int handle_pid_link_task(struct rpc_desc *desc, void *_msg, size_t size)
{
	...
	DEBUG(DBG_PID, 1, "Begin - PID: %d, NodeID: %d\n",
	      msg->pid, kerrighed_node_id);

	task_obj = kh_task_readlock(msg->pid);

	pid = kh_get_pid(msg->pid);
	if (!pid) {
		r = -ENOMEM;
		goto out;
	}
...
{
	hook_register(&kh_get_pid, kcb_get_pid);
	hook_register(&kh_end_get_pid, kcb_end_get_pid);
	hook_register(&kh_put_pid, kcb_put_pid);
	return;
}
...
void pid_management_start(void)
{
...
#endif
	pid_kddm_obj_cachep = kmem_cache_create("pid_kddm_obj",
						sizeof(struct pid_kddm_object),
						0, cache_flags,
						NULL, NULL);
	...
	rpc_register_int(PROC_CANCEL_PID_RESERVATION, handle_cancel_pid_reservation, 0);
}
