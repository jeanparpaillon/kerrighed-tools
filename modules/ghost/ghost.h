...
#include <ghost/network_ghost.h>
#include <ghost/file_ghost.h>
#include <ghost/memory_ghost.h>
 ...
 */
static inline
int __must_check ghost_write ( ghost_t *ghost,
			       const void *buff, size_t length )
{
  int r = 0 ;

  BUG_ON (ghost == NULL);
  BUG_ON (ghost->ops == NULL);
  BUG_ON (ghost->ops->write == NULL);
  BUG_ON (buff == NULL);
 ...
 */
static inline
int __must_check ghost_read ( ghost_t *ghost,
...
int nazgul_ghost_init(void) ;



/* Ghost Finalization. */

void nazgul_ghost_finalize(void) ;
