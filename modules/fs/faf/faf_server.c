		        ...
		        void *msgIn, size_t size)
{
	...
	DEBUG ("server", 2, -1, kerrighed_node_id, msg->server_fd, -1UL,
	       NULL, fcheck_files(current->files, msg->server_fd),
	       FAF_LOG_ENTER, 0);
	DEBUG_RPC(desc, "%d handle_faf_accept:b\n", current->pid);
	...
	DEBUG_RPC(desc, "%d handle_faf_accept: av pack\n", current->pid);
	...
	DEBUG_RPC(desc, "%d handle_faf_accept:e\n", current->pid);
...
void handle_faf_setsockopt (struct rpc_desc *desc,
			    void *msgIn, size_t size)
{
	faf_setsockopt_msg_t *msg = msgIn;
	int r, err;
...
void handle_faf_getsockopt (struct rpc_desc *desc,
			    void *msgIn, size_t size)
{
	faf_getsockopt_msg_t *msg = msgIn;
	int r, err;
