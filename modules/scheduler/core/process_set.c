/*
 *  Kerrighed/modules/scheduler/core/process_set.c
 *
					      ...
					      const char *name)
{
	...
	struct string_list_object *global_ids = NULL;
	pid_t id;
	int err;
			     ...
			     struct pid *pid, enum pid_type type)
{
	 ...
	 * linked until we release the subsystem semaphore, since all
	 * link/unlink are done in make_item/drop_item operations or in
	 * import_process_set_links with the semaphore held. */
	down(&krg_scheduler_subsys.su_sem);

	nr_links = 0;
	/* No need to acquire process_set_link_lock since all mutations of
	 * process set links are protected by krg_scheduler_subsys.su_sem */
...
/* 	hlist_for_each_entry(pset_el, pos, &pid->process_sets[type], pid_node) */
/* 		elements[nr++] = pset_el; */
	BUG_ON(nr != nr_links);
			     ...
			     struct pid *pid, enum pid_type type)
{
	...
	for (nr = 0; nr < nr_links; nr++) {
		...
		/* Taking the subsystem semaphore blocks all other calls of
