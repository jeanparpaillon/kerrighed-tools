/*
 *  Kerrighed/modules/scheduler/core/krg_sched_info.c
 *
...
int krg_sched_module_info_register(struct krg_sched_module_info_type *type)
{
	...
	/*
	 * Matches smp_read_barrier_depends() in kcb_copy_sched_info(), which
...
/*
 * Algorithm similar to kcb_copy_sched_info()'s normal path, with only new
 * module infos.
 */
