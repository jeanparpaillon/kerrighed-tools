/*
 *  Kerrighed/modules/scheduler/core/global_config.c
 *
...
static struct dentry * get_child_dentry(const char *child_name)
{
	...
	if (last_child_comp) {
		...
		nd.mnt = mntget(scheduler_fs_mount);
		nd.dentry = d_dir;
		nd.last_type = LAST_ROOT;
		nd.flags = LOOKUP_PARENT;
		nd.depth = 0;

		err = path_walk(child_name, &nd);
		if (err)
			return ERR_PTR(err);

		d_dir = dget(nd.dentry);
		path_release(&nd);
 ...
 *
 * @param prev_rootmnt valid pointer to a struct vfsmount pointer
 * @param prev_root    valid pointer to a struct dentry pointer
 *
 ...
 */
static void chroot_to_scheduler_subsystem(struct vfsmount **prev_rootmnt,
					  struct dentry **prev_root)
{
	...
	*prev_rootmnt = mntget(current->fs->rootmnt);
	*prev_root = dget(current->fs->root);
	read_unlock(&current->fs->lock);

	set_fs_root(current->fs,
		    scheduler_fs_mount,
		    krg_scheduler_subsys.su_group.cg_item.ci_dentry);
}

static void chroot_restore(struct vfsmount *prev_rootmnt,
			   struct dentry *prev_root)
{
	set_fs_root(current->fs, prev_rootmnt, prev_root);
	mntput(prev_rootmnt);
	dput(prev_root);
}
					...
					const char *old_name)
{
	...
	chroot_to_scheduler_subsystem(&old_rootmnt, &old_root);
	err = vfs_symlink(d_child->d_parent->d_inode, d_child,
			  old_name, S_IALLUGO);
	chroot_restore(old_rootmnt, old_root);
 ...
 *
 * @return		valid pointer or NULL to be passed to
 *			global_config_allow_link_name_end or
 *			global_config_allow_link_name_error, or error
 */
struct string_list_object *
global_config_allow_link_name_begin(struct config_item *parent,
				    const char *name,
...
/*
 * Same as global_config_allow_link_name_end() below, except that concurrent
 * operations are kept disabled.
 * Caller is responsible for calling __global_config_allow_link_name_end()
 * afterwards.
 */
int __global_config_allow_link_name_commit(struct string_list_object *list,
					   struct config_item *parent,
					   struct global_config_item *item,
					   const char *name,
					   struct config_item *target)
...
/*
 * Last step of a global allow_link_name(). Re-enables concurrent operations.
 *
 * @param list		pointer returned by global_config_allow_link_name_begin()
 */
void __global_config_allow_link_name_end(struct string_list_object *list)
{
	__create_end(list);
}
 ...
 * @return		0 on success, or error
 */
int global_config_allow_link_name_end(struct string_list_object *list,
				      struct config_item *parent,
				      struct global_config_item *item,
				      const char *name,
				      struct config_item *target)
{
	...
	err = __global_config_allow_link_name_commit(list,
						     parent,
						     item,
						     name,
						     target);
	__global_config_allow_link_name_end(list);
 ...
 * global_config_allow_link_name_end
 *
 * @param list		pointer returned by global_config_allow_link_name_begin
 * @param name		name of the new link
 * @param target	target item of the new link
 */
void global_config_allow_link_name_error(struct string_list_object *list,
					 const char *name,
					 struct config_item *target)
{
	create_error(list, name);
...
static struct config_item * get_item(const char *path)
{
	...
	down(&krg_scheduler_subsys.su_sem);
	do {
		parent = to_config_group(child);
		next_root = strchr(parent_root + 1, '/');
		if (next_root)
			*next_root = '\0';
		child = config_group_find_obj(parent, parent_root + 1);
		if (!child)
			break;
	...
	up(&krg_scheduler_subsys.su_sem);
