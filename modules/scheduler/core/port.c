/*
 *  Kerrighed/modules/scheduler/core/port.c
 *
 ...
 */
static int scheduler_port_allow_link_name(struct config_item *src,
					  ...
					  const char *name)
{
	...
	list = global_config_allow_link_name_begin(src, name, target);
	if (IS_ERR(list)) {
		err = PTR_ERR(list);
	...
	err = global_config_allow_link_name_end(list,
						src,
...
static struct configfs_item_operations port_item_ops = {
	...
	.allow_link_name = scheduler_port_allow_link_name,
