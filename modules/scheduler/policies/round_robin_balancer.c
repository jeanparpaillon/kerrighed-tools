/*
 *  Kerrighed/modules/scheduler/policies/round_robin_balancer.c
 *
 ...
 *
 * The first node chosen is the local node, and subsequent nodes are obtained in
 * node id order.
 *
 * E.g: let a cluster have node ids 0 2 3 5
 *      - from node 0, the sequence will be 0 2 3 5 0 2 3 5 0 ...
 *      - from node 2, the sequence will be 2 3 5 0 2 3 5 0 2 ...
 *
				   ...
				   struct task_struct *parent)
{
	...
	rrb_lock(rrb);
	if (rrb->last_target == KERRIGHED_NODE_ID_NONE) {
		node = kerrighed_node_id;
		if (!krgnode_isset(node, nodes))
			node = next_krgnode_in_ring(node, nodes);
	} else {
		node = next_krgnode_in_ring(rrb->last_target, nodes);
	}
