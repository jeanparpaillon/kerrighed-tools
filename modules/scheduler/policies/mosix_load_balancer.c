/*
 *  Kerrighed/modules/scheduler/policies/mosix_load_balancer.c
 *
...
#include <scheduler/core/policy.h>
#include <scheduler/core/port.h>
#include <scheduler/core/scheduler.h>
#include <scheduler/core/process_set.h>
...
/* Give root privileges for migration requests */
static struct caller_creds lb_creds = {
	.caller_uid = 0,
	.caller_euid = 0
};
...
static struct task_struct * find_target_task(struct mosix_load_balancer *lb)
{
...
static void balance(struct mosix_load_balancer *lb, unsigned long current_load)
{
	...
	target_task = find_target_task(lb);
	if (!target_task)
		goto out;
	...
	__migrate_linux_threads(target_task, MIGR_LOCAL_PROCESS, target_node,
				&lb_creds);
			 ...
			 const krgnodemask_t *nodes)
{
	...
	process_set_do_each_process(t, processes) {
		...
		err = __migrate_linux_threads(t, MIGR_LOCAL_PROCESS,
					      node, &lb_creds);
		if (err && err != -EALREADY)
			printk(KERN_WARNING "mosix_load_balancer:"
