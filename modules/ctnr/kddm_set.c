				    ...
				    kddm_set_id_t set_id)
{
	...
	DEBUG("kddm_lookup", 2, ns->id, set_id, -1L, KDDM_LOG_ENTER, 0);
...
check_no_lock:
	...
	DEBUG("kddm_lookup", 2, ns->id, set_id, -1L, KDDM_LOG_EXIT, 0);
				...
				void *msg, size_t size)
{
	...
	BUG_ON(!krgnode_online(desc->client));

	DEBUG("usage", 2, kddm_id.ns_id, kddm_id.set_id, -1L, KDDM_LOG_ENTER,
	      0);
	...
	DEBUG("usage", 2, kddm_set->ns->id, kddm_set->id, -1L, KDDM_LOG_EXIT,
	      0);
			       ...
			       void *_msg, size_t size)
{
	...
	DEBUG("kddm_lookup", 2, kddm_id.ns_id, kddm_id.set_id, -1L,
	      KDDM_LOG_LOOK_UP, desc->client);
	...
	DEBUG("kddm_lookup", 4, kddm_id.ns_id, kddm_id.set_id, -1L,
	      KDDM_LOG_FOUND_SET, kddm_set);
				...
				void *msg)
{
	...
	DEBUG("creation", 2, kddm_id.ns_id, kddm_id.set_id, -1L,
	      KDDM_LOG_ENTER, 0);
...
int handle_req_kddm_set_destroy(struct rpc_desc* desc,
				void *msg, size_t size){
	return __handle_req_kddm_set_destroy(desc->client, msg);
}
...
int _increment_kddm_set_usage(struct kddm_set * kddm_set)
{
	...
	DEBUG("usage", 2, kddm_set->ns->id, kddm_set->id, -1L,
	      KDDM_LOG_ENTER, 0);
...
void kddm_set_init()
{
	...
	kddm_set_cachep = kmem_cache_create("kddm_set",
					    sizeof(struct kddm_set),
					    0, SLAB_PANIC, NULL, NULL);

	kddm_tree_cachep = kmem_cache_create("kddm_tree",
					     sizeof(struct kddm_tree),
					     0, SLAB_PANIC, NULL, NULL);

	kddm_tree_lvl_cachep = kmem_cache_create("kddm_tree_lvl",
						 sizeof(struct kddm_tree_lvl),
						 0, SLAB_PANIC, NULL, NULL);
