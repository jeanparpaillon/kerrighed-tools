...
void create_kddm_proc_dir (void)
{
  ...
  create_seq_entry("kddminfo", S_IRUGO, &proc_kddminfo_operations);
}
...
/****************************************************************************/
/*                                                                          */
/*                     /proc/kerrighed/<pid>  Management                     */
/*                                                                          */
/****************************************************************************/



extern int (*kh_proc_kddm)(struct task_struct *task, char *buffer);


static int print_set(char *buffer, krgnodemask_t *set)
{
	int i, len = 0;

	for (i = 0; i < KERRIGHED_MAX_NODES; i++)
		if (NODE_IN_SET (set, i))
			len += sprintf (buffer +len, "%d ", i);

	return len;
}

static int print_wq(char *buffer, wait_queue_head_t *q)
{
 ...
 *  @return  Number of bytes written.
 */
int kcb_proc_kddm (struct task_struct *task,
		   char *buffer)
{
	...
	len += print_set(buffer + len, &obj_entry->master_obj.copyset);
	len += sprintf (buffer + len, "\n  * Remove set: ");
	len += print_set(buffer + len, &obj_entry->master_obj.copyset);
	len += sprintf (buffer + len, "\n  * Waiting processes: ");
	len += print_wq (buffer + len, &obj_entry->waiting_tsk);
	len += sprintf (buffer + len, "\n");
done:
