...
void enable_all_netdev(void){
	...
	for(netdev=dev_base;netdev;netdev=netdev->next){
		...
		if(!netdev->hard_header){
			printk(" hard_header undefined! netdev skipped!\n");
			continue;
		}
...
int comlayer_init(void)
{
	...
	tipc_own_addr = tipc_addr(1, 1, kerrighed_node_id+1);
	tipc_core_start_net();
	...
	for_each_possible_cpu(i){
		...
		*/
		*send_ref = tipc_createport_raw((void*)i,
	...
	kh_node_reachable = node_reachable;
	kh_node_unreachable = node_unreachable;
