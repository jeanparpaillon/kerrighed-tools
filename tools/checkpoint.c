...
{
	printf("Usage: %s [options] <pid> DIRECTORY\n"
	       "\n"
	       "Mutually Exclusive Options:\n"
	       "  Without any of these options, freeze and checkpoint the application in directory DIRECTORY.\n"
	       "  -f|--freeze             Freeze the application\n"
	       "  -u|--unfreeze [signal]  Unfreeze the application\n"
	       "  -c|--ckpt-only          Checkpoint a frozen application and store checkpoint in directory DIRECTORY\n"
...
{
	/*
	 * cannot clean the checkpoint dir easily since it has been
	 * given by the user.
